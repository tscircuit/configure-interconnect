 ...
 * For X pins that are paired (share the same net), both are included.
 */
export const getOuterPinNets = (
  ...
  for (const [connectivityKey, netGroup] of netGroups.entries()) {
    // Find ALL outer pins (C or X) in this net
    const outerPorts = netGroup.ports.filter((p) => {
      const kind = pinKind(p)
      return kind === "C" || kind === "X"
    })

    if (outerPorts.length === 0) continue

    // Create an entry for each outer pin
    for (const outerPort of outerPorts) {
